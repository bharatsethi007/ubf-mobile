...
import {
  ...
  uploadMeetingCard,
  type MeetingCard,
} from '@/lib/meetingCardsApi'

export default function MeetingCards({ meetingId }: { meetingId: string }) {
  const [photos, setPhotos] = useState<MeetingCard[]>([])
  const [busy, setBusy] = useState(false)

  useEffect(() => {
    let active = true
    listMeetingCards(meetingId)
      .then((p) => {
  ...
  async function handle(source: 'camera' | 'library') {
    ...
    try {
      ...
      const photo = await uploadMeetingCard(
        meetingId,
        asset.base64,
  ...
  function onAdd() {
    Alert.alert('Add business card', undefined, [
  ...
  return (
    <View style={{ gap: spacing.md, marginTop: spacing.sm }}>
      <View style={styles.head}>
        <Text variant="label">Business cards</Text>
        <Pressable
          onPress={onAdd}
          disabled={busy}
        ...
        >
          {busy ? (
            ...
            <>
              ...
              <Text style={{ color: colors.navy, fontWeight: '600', fontSize: 13 }}>Add card</Text>
            </>
          )}
        </Pressable>
      </View>
      ...
      {photos.length === 0 ? (
        <Text variant="muted">No business cards yet.</Text>
      ) : (
